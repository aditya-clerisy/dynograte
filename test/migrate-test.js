...
describe("Migration table test", function () {
  ...
  beforeEach(() => {
    ...
    return _createDynamoTable(dynamodb, dummySchema, console).then(() => {
  ...
  describe("Migration table already exists", () => {
    ...
    afterEach(() => {
      return _deleteDynamoTable(dynamodb, randomMigrationTableName).then(() => {
  ...
  describe("Failing migrations", () => {
    ...
    it("should remove row from dynamodb if a migration fails", () => {
      let migrationDir = path.resolve(
        __dirname,
        "./failing-dynamodb-migrations"
      );
    ...
    it("it should retry migrations with retry set to true", function () {
      let migrationDir = path.resolve(
        __dirname,
        "./failing-retry-boolean-dynamodb-migrations"
      );
      let migrationFile = require(path.resolve(
        migrationDir,
        "2016-09-07-10-48-28-update-users-table"
      ));
    ...
    it("it should retry migrations with retry set to object", () => {
      let migrationDir = path.resolve(
        __dirname,
        "./failing-retry-options-dynamodb-migrations"
      );
      let migrationFile = require(path.resolve(
        migrationDir,
        "2016-09-07-10-48-28-update-users-table"
      ...
      return new Promise((resolve, reject) => {
        return dynograte
          ...
          .catch((err) => {
            expect(err.message).to.equal("This migration failed!");
            expect(migrationFile.timesRun).to.equal(
              migrationFile.retry.maxAttempts
            );
    ...
    it("it should retry migrations the next time migrations run if runNextMigrateAfterFail set to true", () => {
      let migrationDir = path.resolve(
        __dirname,
        "./failing-runNextMigrateAfterFail-dynamodb-migrations"
      );
      let migrationFile = require(path.resolve(
        migrationDir,
        "2016-09-07-10-48-28-update-users-table"
      ...
      return new Promise((resolve, reject) => {
        return dynograte
          ...
          .catch((err) => {
            ...
            return _scanDynamoTable(dynamodb, randomMigrationTableName).then(
              (res) => {
                expect(res.Items[0].failed).to.deep.equal({ BOOL: true });
                ...
                return dynograte
                  ...
                  .then(() => {
                    ...
                    return _scanDynamoTable(
                      dynamodb,
                      randomMigrationTableName
                    ).then((res) => {
                      expect(res.Items[0].failed).to.deep.equal({
                        BOOL: false,
                      });
                      expect(res.Items[0].pending).to.deep.equal({
                        BOOL: false,
                      });
                      resolve();
                    });
