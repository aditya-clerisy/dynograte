...
describe("Generate migration file name test", function () {
  it("should generate migration file name with date", () => {
    ...
    let migrationFileName = generateMigrationFileName(
      "/Project/test",
      migrationName
    );
    let date = convertDate(
      migrationFileName.substring(
    ...
    expect(
      migrationFileName.substr(
        migrationFileName.length - (migrationName.length + 3)
      )
    ).to.equal("test-migration.js");
  });
});
